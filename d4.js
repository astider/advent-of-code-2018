...
const logsobj = logs.map(l => {
  const split = l.split("]");
  const date = split[0].split("[")[1];
  const guard = split[1].indexOf('Guard') >= 0 ? split[1].split(" ")[0] + split[1].split(" ")[1] + split[1].split(" ")[2] : false;
  const event = guard ? split[1].split(" ").slice(3).join(' ') : split[1].split(" ").slice(1).join(' ');
  return {
    date,
    guard,
    event
...
const sorted = logsobj.sort((a, b) => {
  return new Date(a.date) - new Date(b.date);
});

const stat = sorted.reduce((acc, data) => {
  // console.log('+++++++++++++++++++++');
  // console.log(data.guard, data.event);
  if (data.guard) {
    if (!acc[data.guard]) {
      acc[data.guard] = [];
      acc[data.guard][60] = -1;
    }
    acc['latestGuard'] = data.guard;
    console.log('set latest guard', acc.latestGuard);
  } else {
    // console.log(data);
    if (data.event === 'falls asleep' /* && acc[acc.latestGuard][60] === -1 */) {
      // console.log('sleep');
      acc[acc.latestGuard][60] = (new Date(data.date)).getMinutes();
      console.log(`guard ${acc.latestGuard} sleep at ${acc[acc.latestGuard][60]}`);
    }
    else if (data.event === 'wakes up' && acc[acc.latestGuard][60] !== -1) {
      let startSleep = acc[acc.latestGuard][60];
      const sleep = startSleep;
      const wake = (new Date(data.date)).getMinutes();
      // console.log(startSleep, wake);
      while (startSleep < wake) {
        if (!acc[acc.latestGuard][startSleep]) acc[acc.latestGuard][startSleep] = 1;
        else acc[acc.latestGuard][startSleep] = acc[acc.latestGuard][startSleep] + 1;
        // console.log(acc.latestGuard[i]);
        startSleep++;
      }
      // console.log(acc[acc.latestGuard]);
      console.log(`guard ${acc.latestGuard} sleep at [${sleep}] wake up at [${wake}]`);
      acc[acc.latestGuard][60] = -1;
    }
  }
  // console.log('-----------------');
  return acc;
